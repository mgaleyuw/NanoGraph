...
createTextSummary <- function(indf, outputname=NA, append=FALSE){
  ...
  if(! is.na(outputname)){
    writeline=paste(coverageAll, coverageNonN, qunatiles["Min."], quantiles["Max."], quantiles["Mean"], quantiles["1st Qu."], quantiles["Median"], quantiles["3rd Qu."], sep="\t")
    if(append){
      write(writeline,file=outputname.txt,append=TRUE)
    } else {
      outfile=file(outputname, "w")
      header=paste("coverage", "coverageNonN", "minRead", "maxRead", "meanRead", "N25", "N50", "N75", sep="\t")
      writeLines(header, con=outfile)
      writeLines(writeline, con=outfile)
      close(outfile)
    }
  } else {
    return( c("coverage"=coverageAll, "coverageNonN"=coverageNonN, "minRead"=quantiles["Min."], "maxRead"=quantiles["Max."], "meanRead"=quantiles["Mean"], "N25"=quantiles["1st Qu."], "N50"=quantiles["Median"], "N75"=quantiles["3rd Qu."]) )
  }
}
...
#'@return a ggplot2 plot
#'@export
plotCountBases <- function(df, bin.num, plotQual=TRUE, qual.bins=c(10, 15, 20, 25, 30, 35, 40, 45, 50, 55, 60), binWidth=8000, barColor="#8ec462"){
